...
```json
{
  "standings": [
    {
      "subject_kind": "string — 'pair' or 'player'; discriminates the row shape below",
      "rank": "integer — position in the table; tied subjects share the same rank",
      "pair_id": "string (UUID) — present when subject_kind == 'pair'",
      "player1_nickname": "string — present when subject_kind == 'pair'",
      "player2_nickname": "string — present when subject_kind == 'pair'",
...
- **v3 row-count semantics:** the number of rows depends on the league's `LeagueRules`:
  ...
  |---|---|
  | `(pair, OTPP=true)` | exactly 1 (the player's single pair row) |
  | `(pair, OTPP=false)` and the player belongs to N pairs | N rows (one per pair) |
  ...
  "matches": [
    {
      ...
      "pair1_player1_nickname": "string",
      "pair1_player2_nickname": "string",
      "pair2_player1_nickname": "string",
      "pair2_player2_nickname": "string",
      "pair1_score": "string — non-negative integer as string (e.g. '6')",
      "pair2_score": "string — non-negative integer as string (e.g. '3')",
...
- **For DELETE_MATCH handler:** this endpoint is called to resolve a single `match_id` from the four player nicknames extracted from the user's message. Match using case-insensitive nickname comparison, considering both player orderings within each pair (player1/player2 positions are not guaranteed to be consistent). If no match is found for the given nicknames, the handler should return an ERROR response (status_code 502). If multiple matches exist for the same player combination, use the most recent one and note the ambiguity in `server_message`.
...
- **Description**: Returns the list of all registered players and pairs in the league.
- **Used by Intents**: GET_ROSTER, EDIT_PLAYER_NICKNAME (supplementary — resolves player_id from current_nickname), DELETE_PAIR (supplementary — resolves pair_id from player nicknames)
...
- **For DELETE_PAIR handler:** look up the pair by matching both `player1_nickname` and `player2_nickname` (case-insensitive, in either order) in the `pairs` list to resolve the `pair_id`. If no matching pair is found, return an ERROR response (status_code 502).
...
- Because of the second path, `GET /leagues/{league_id}/roster` may return `players` entries that have **no pair and zero matches** (pre-registered roster members). The response shape is unchanged — a Player with no pair simply does not appear in the `pairs` array. Handlers that compute "matches played by player" or similar metrics should not assume `players ⊆ pair-membership`.
